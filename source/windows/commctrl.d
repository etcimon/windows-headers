...
BOOL ListView_GetItemRect(HWND w, int i, LPRECT p, int c) {
	if (p) p.left = c;
	return cast(BOOL) SendMessage(w, LVM_GETITEMRECT, i, p ? cast(LPARAM) p : 0);
}
...
static if (_WIN32_IE >= 0x300) {
	...
	BOOL ListView_GetSubItemRect(HWND w, int i, int isi, int c, LPRECT p) {
		if (p) { p.left = c; p.top = isi; }
		return cast(BOOL) SendMessage(w, LVM_GETSUBITEMRECT, i, p ? cast(LPARAM) p : 0);
	}
